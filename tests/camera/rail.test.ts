...
test('sampled every centimetre, the camera stays inside the stops and the view never jumps', () => {
  ...
  expect(rail.length / samples).toBeLessThanOrEqual(0.01);
...
test('arc length is even: a centimetre of u is a centimetre of walk, with no stalled turns', () => {
  const rail = new Rail(plan.rail, plan.path);
  const samples = Math.ceil(rail.length / 0.01);
  let worst = 0;
  let least = Infinity;
  ...
  for (let i = 1; i <= samples; i++) {
    ...
    least = Math.min(least, position.distanceTo(previous) / (rail.length / samples));
