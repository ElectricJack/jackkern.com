...
test('the camera rail crosses each doorway it meets through the opening, not the frame', () => {
  ...
  // The curve the camera rides, as src/main.ts builds it: a spline through the whole walk.
  // A spline through the viewpoints alone runs through the walls between them instead.
  ...
  // The walk goes out through the bay fill() opens, so the rail meets all eight doorways and
  // clears every one, crossing on centre and 1.70 m up — 2.70 m at quilt-trader's entry, whose
  // wall stands on the floor below the courtyard the rail steps down from.
